...
class DataModule(pl.LightningDataModule):
    def __init__(self, training_path: str = './', validation_path: str = './', test_path: str = './', batch_size: int = 5, num_workers: int = 2, size = 256, means = [0.7000, 0.6413, 0.6352], std = [0.2529, 0.2519, 0.2450]):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        self.DS_train = datasets.ImageFolder(root = self.training_dir,
                                  transform = img_tf)
        self.DS_validation = datasets.ImageFolder(root = self.validation_dir, transform = img_tf_val)
