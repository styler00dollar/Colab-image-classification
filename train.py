...
def main():
    ...
    dm = DataModule(training_path=args.training_path, validation_path=args.validation_path, test_path=args.validation_path, num_workers = 16, size = 256, batch_size=args.batch_size, means=args.means, std=args.std)
    model = CustomTrainClass(model_train=args.model_train, num_classes=args.num_classes, diffaug_activate=args.diffaug_activate, policy=args.policy)
