...
def get_roll_masks(H, W, window_size, shift_size):
    ...
    attn_mask_tl = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(
    ...
    mask_windows = mask_windows.view(-1, window_size * window_size)
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
    attn_mask_tr = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(
        attn_mask == 0, float(0.0)
    )
    ...
    mask_windows = mask_windows.view(-1, window_size * window_size)
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
    attn_mask_bl = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(
        attn_mask == 0, float(0.0)
    )
    ...
    mask_windows = mask_windows.view(-1, window_size * window_size)
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
    attn_mask_br = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(
        attn_mask == 0, float(0.0)
    )

    # append all
    attn_mask_all = torch.cat(
        (attn_mask_tl, attn_mask_tr, attn_mask_bl, attn_mask_br), -1
    )
...
def get_relative_position_index(q_windows, k_windows):
    ...
    relative_coords = (
        coords_flatten_q[:, :, None] - coords_flatten_k[:, None, :]
    )  # 2, Wh_q*Ww_q, Wh_k*Ww_k
    relative_coords = relative_coords.permute(
        1, 2, 0
    ).contiguous()  # Wh_q*Ww_q, Wh_k*Ww_k, 2
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.expand_size > 0 and focal_level > 0:
            ...
            self.window_size_of_key = (
                self.window_size[0] * self.window_size[1]
                if self.expand_size == 0
                else (
                    4 * self.window_size[0] * self.window_size[1]
                    - 4
            ...
            )  # Wh*Ww, nH, nSurrounding
            trunc_normal_(self.relative_position_bias_table_to_neighbors, std=0.02)
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level - 1):
                ...
                trunc_normal_(relative_position_bias_table_to_windows, std=0.02)
                ...
                relative_position_index_k = get_relative_position_index(
                    self.window_size, to_2tuple(self.focal_window + 2**k - 1)
                )
                self.register_buffer(
                    "relative_position_index_{}".format(k), relative_position_index_k
                )
    ...
    def forward(self, x_all, mask_all=None):
        ...
        (q_windows, k_windows, v_windows) = map(
            lambda t: window_partition(t, self.window_size[0])
            .view(
                -1,
        ...
        if self.expand_size > 0 and self.focal_level > 0:
            (k_tl, v_tl) = map(
                lambda t: torch.roll(
                    t, shifts=(-self.expand_size, -self.expand_size), dims=(1, 2)
                ),
                (k, v),
            )
            (k_tr, v_tr) = map(
            ...
            k_rolled = torch.cat(
                (k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows), 1
            ).transpose(1, 2)
            v_rolled = torch.cat(
                (v_tl_windows, v_tr_windows, v_bl_windows, v_br_windows), 1
            ).transpose(1, 2)
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                ...
                unfolded_mask = (
                    self.unfolds[k](mask.unsqueeze(0).unsqueeze(1))
                    .view(
                        1,
                        1,
        ...
        attn[:, :, :window_area, :window_area] = attn[
            :, :, :window_area, :window_area
        ] + relative_position_bias.unsqueeze(0)

        # add relative position bias for patches inside a window
        if self.expand_size > 0 and self.focal_level > 0:
            attn[:, :, :window_area, window_area:window_area_rolled] = (
                attn[:, :, :window_area, window_area:window_area_rolled]
                + self.relative_position_bias_table_to_neighbors
            )
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                ...
                relative_position_index_k = getattr(
                    self, "relative_position_index_{}".format(k)
                )
                relative_position_bias_to_windows = (
                    self.relative_position_bias_table_to_windows[k][
                        :, relative_position_index_k.view(-1)
                    ].view(
                        -1,
                        self.window_size[0] * self.window_size[1],
                ...
                attn[
                    ...
                    :,
                    :window_area,
                    offset : (offset + (self.focal_window + 2**k - 1) ** 2),
                ] = attn[
                    :,
                    :,
                    :window_area,
                    offset : (offset + (self.focal_window + 2**k - 1) ** 2),
                ] + relative_position_bias_to_windows.unsqueeze(
                    0
                )
...
class FocalTransformerBlock(nn.Module):
            ...
            for k in range(self.focal_level - 1):
                ...
                if self.pool_method == "fc":
                    self.pool_layers.append(
                        nn.Linear(window_size_glo * window_size_glo, 1)
                    )
                    self.pool_layers[-1].weight.data.fill_(
                        1.0 / (window_size_glo * window_size_glo)
                    )
    ...
    def forward(self, x):
        ...
        if self.focal_level > 1 and self.pool_method != "none":
            ...
            for k in range(self.focal_level - 1):
                ...
                if self.pool_method == "mean":
                    x_windows_pooled = x_windows_noreshape.mean(
                        [3, 4]
                    )  # B, nWh, nWw, C
                elif self.pool_method == "max":
                    x_windows_pooled = (
                        x_windows_noreshape.max(-2)[0].max(-2)[0].view(B, nWh, nWw, C)
                    )  # B, nWh, nWw, C
                elif self.pool_method == "fc":
                    x_windows_noreshape = x_windows_noreshape.view(
                        B, nWh, nWw, window_size_glo * window_size_glo, C
                    ).transpose(
                        3, 4
                    )  # B, nWh, nWw, C, wsize**2
                    ...
                    x_windows_noreshape = (
                        x_windows_noreshape.view(
                            -1, window_size_glo, window_size_glo, C
                        )
                        .permute(0, 3, 1, 2)
                        .contiguous()
                    )  # B * nw * nw, C, wsize, wsize
                    x_windows_pooled = self.pool_layers[k](x_windows_noreshape).view(
                        B, nWh, nWw, C
                    )  # B, nWh, nWw, C
        ...
        if self.shift_size > 0:
            ...
            x = shifted_x
        x = (
            x[:, : self.input_resolution[0], : self.input_resolution[1]]
            .contiguous()
            .view(B, -1, C)
        )
        ...
        x = shortcut + self.drop_path(
            x if (not self.use_layerscale) else (self.gamma_1 * x)
        )
        x = x + self.drop_path(
            self.mlp(self.norm2(x))
            if (not self.use_layerscale)
            else (self.gamma_2 * self.mlp(self.norm2(x)))
        )
    ...
    def flops(self):
        ...
        flops += nW * self.attn.flops(
            self.window_size * self.window_size, self.window_size, self.focal_window
        )
