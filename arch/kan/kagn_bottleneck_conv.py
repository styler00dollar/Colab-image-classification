# https://github.com/IvanDrokin/torch-conv-kan/blob/851ae0ca05632f8e8f1785e08d68ba88109afbd0/kan_convs/kagn_bottleneck_conv.py
    ...
    def __init__(
        ...
        **norm_kwargs
        ...
        if inner_dim < min_internal:
            self.inner_dim = min(
                min_internal, input_dim // groups, output_dim // groups
            )
        else:
        ...
        self.layer_norm = nn.ModuleList(
            [norm_class(output_dim // groups, **norm_kwargs) for _ in range(groups)]
        )
...
class MoEBottleNeckKAGNConvND(nn.Module):
    ...
    def __init__(
        ...
        **norm_kwargs
        ...
        self.w_gate = nn.Parameter(
            torch.zeros(self.inner_dim * groups, num_experts), requires_grad=True
        )
        self.w_noise = nn.Parameter(
            torch.zeros(self.inner_dim * groups, num_experts), requires_grad=True
        )
    ...
    def forward(self, x, train=True, loss_coef=1e-2):
        ...
        for group_ind, (_xb, _xe) in enumerate(
            zip(bases, torch.split(y, self.inner_dim, dim=1))
        ):
