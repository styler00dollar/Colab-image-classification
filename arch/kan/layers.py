# https://raw.githubusercontent.com/IvanDrokin/torch-conv-kan/851ae0ca05632f8e8f1785e08d68ba88109afbd0/kans/layers.py
...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        bases = (
            ((x_uns >= grid[:, :-1]) & (x_uns < grid[:, 1:])).to(x.dtype).to(x.device)
        )
...
class KALNLayer(nn.Module):  # Kolmogorov Arnold Legendre Network (KAL-Net)
    def __init__(
        self, input_features, output_features, degree=3, base_activation=nn.SiLU
    ):
...
class BernsteinKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, degree, act=nn.SiLU):
        ...
        nn.init.normal_(
            self.bernstein_coeffs, mean=0.0, std=1 / (input_dim * (degree + 1))
        )
        nn.init.xavier_uniform_(self.base_weights)
    ...
    def forward(self, x):
        ...
        y = torch.einsum(
            "bid,iod->bo", bernsteins, self.bernstein_coeffs
        )  # shape = (batch_size, outdim)
        y = y.view(-1, self.outdim)
...
class ReLUKANLayer(nn.Module):
    ...
    def __init__(
        ...
        self.phase_low = nn.Parameter(
            phase_low[None, :].expand(input_size, -1), requires_grad=train_ab
        )
        # modification here, and: `phase_height` to `phase_high`
        self.phase_high = nn.Parameter(
            phase_high[None, :].expand(input_size, -1), requires_grad=train_ab
        )
