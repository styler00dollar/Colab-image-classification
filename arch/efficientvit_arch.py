# https://github.com/microsoft/Cream/blob/main/EfficientViT/classification/model/build.py
...
class PatchMerging(torch.nn.Module):
    def __init__(self, dim, out_dim, input_resolution):
        ...
        self.conv2 = Conv2d_BN(
            hid_dim, hid_dim, 3, 2, 1, groups=hid_dim, resolution=input_resolution
        )
        self.se = SqueezeExcite(hid_dim, 0.25)
        self.conv3 = Conv2d_BN(
            hid_dim, out_dim, 1, 1, 0, resolution=input_resolution // 2
        )
...
class CascadedGroupAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_heads):
            qkvs.append(
                Conv2d_BN(
                    dim // (num_heads), self.key_dim * 2 + self.d, resolution=resolution
                )
            )
            dws.append(
                Conv2d_BN(
...
class EfficientViTBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dw0 = Residual(
            Conv2d_BN(
                ed, ed, 3, 1, 1, groups=ed, bn_weight_init=0.0, resolution=resolution
            )
        )
...
class EfficientViT(torch.nn.Module):
    def __init__(
        ...
        self.patch_embed = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            Conv2d_BN(
                embed_dim[0] // 2, embed_dim[0], 3, 2, 1, resolution=resolution // 8
            ),
        )
...
def EfficientViT_M0(
    ...
    model = EfficientViT(
        num_classes=num_classes, distillation=distillation, **model_cfg
    )
    if pretrained:
